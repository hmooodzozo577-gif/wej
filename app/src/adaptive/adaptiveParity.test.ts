...
describe('Phase 14 parity — same final answers must produce the same ranking regardless of question order', () => {
  ...
  it('rankDestinations/scoreDestination depend only on canonical answers', () => {
    const bank = QUESTION_BANKS.tourism;
    // The exact same final answer set, built two ways: entirely
    // direct UI traversal vs a fresh copy of those same canonical
    // values. Ranking never receives answer provenance.
