// Phase 13.5d — TourismInsights: renders metric cards + charts from a
// mocked entry (EN/AR), gracefully renders nothing when the country has
// no snapshot entry, never fabricates a growth/receipts card when the
// underlying series can't support one, and never leaks Price Level
// Index semantics into this card.
...
const japan = WORLD_CATALOG.find((e) => e.id === 'japan')!;
const afghanistan = WORLD_CATALOG.find((e) => e.countryCode === 'AF')!;
...
describe('Phase 13.5d — TourismInsights against the real committed snapshot', () => {
  it('renders nothing while the real committed snapshot has no coverage for this country', async () => {
    const { container } = renderWith(japan, 'en');
    await waitFor(() => expect(tourismInsights.getTourismInsights).toBeDefined());
