...
export function buildNextTurnPrompt(req: NextTurnRequest, retryDiagnostic?: NextTurnRetryDiagnostic): { system: string; user: string } {
  ...
  const system = [
    ...
    'Rules for "free_text": use it only when no small set of choice scenarios can represent the ambiguity without guessing. Never embed answer alternatives such as "A or B" inside a free-text prompt; if alternatives are known, return them as choice options. Keep the prompt short and specific about what you need to know.',
